    ...
    try {
      ...
      if (data.success && data.data) {
        if (type === 'logo') {
          ...
          setSuccess(t('branding.logoUploaded'));
        } else {
          setFavicon(data.data.favicon);
          setSuccess(t('branding.faviconUploaded'));
        }
        setTimeout(() => setSuccess(''), 3000);
      } else {
        setError(typeof data.error === 'string' ? data.error : t('branding.uploadFailed'));
      }
    } catch {
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6 space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">{t('branding.siteName')}</label>
          <input
            type="text"
            value={siteName}
          ...
          />
          <p className="mt-1 text-xs text-gray-500">{t('branding.siteNameHelper')}</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">{t('branding.siteTitle')}</label>
          <input
            type="text"
            value={siteTitle}
            onChange={(e) => setSiteTitle(e.target.value)}
            className="w-full max-w-md px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
            placeholder={t('branding.siteTitlePlaceholder')}
          />
          <p className="mt-1 text-xs text-gray-500">{t('branding.siteTitleHelper')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('branding.logo')}</h2>
          {logo ? (
            <div className="mb-4">
        ...
        {/* Favicon */}
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('branding.favicon')}</h2>
          {favicon ? (
            <div className="mb-4">
