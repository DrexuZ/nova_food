...
export default function SettingsGeneral() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (data.success) {
        setSuccess(t('settingsGeneral.updated'));
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <Link to="/settings" className="text-sm text-primary-600 hover:text-primary-700">{t('settingsGeneral.back')}</Link>
          <h1 className="text-2xl font-bold text-gray-900 mt-1">{t('settingsGeneral.title')}</h1>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" maxLength={3} value={defaultCurrency} onChange={(e) => setDefaultCurrency(e.target.value.toUpperCase())} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder={t('settingsGeneral.currencyPlaceholder')} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('settingsGeneral.currencySymbol')}</label>
            <input type="text" maxLength={5} value={currencySymbol} onChange={(e) => setCurrencySymbol(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder={t('settingsGeneral.symbolPlaceholder')} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">{t('settingsGeneral.currencyPosition')}</label>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">{t('settingsGeneral.googleMapsKey')}</label>
          <input type="password" value={googleMapsApiKey} onChange={(e) => setGoogleMapsApiKey(e.target.value)} className="w-full max-w-md px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" placeholder={t('settingsGeneral.mapsKeyPlaceholder')} />
        </div>
      </div>
    </div>
  );
