...
export default function DesignMedia() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error || t('media.loadFailed'));
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        if (!res.ok) {
          ...
          throw new Error(result.error || t('media.uploadFailed', { name: file.name }));
        }
      }
      await load();
    } catch (e: any) {
  ...
  async function remove(item: MediaAsset) {
    if (!window.confirm(t('media.deleteConfirm', { name: item.originalName }))) return;
    const res = await fetch(`/api/media/${item.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) await load();
    else setError(t('media.deleteFailed'));
  }
  ...
  async function copyUrl(item: MediaAsset) {
    ...
    try {
      ...
      setError(t('media.clipboardError'));
    }
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-semibold text-gray-800">{t('media.title')}</h2>
          <p className="text-sm text-gray-500 mt-1">{t('media.subtitle')}</p>
        </div>
        <button
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">{t('media.fileInfo')}</p>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-gray-500 text-sm py-8 text-center">{t('media.empty')}</p>
      ) : (
            ...
            <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  >
                    {copiedId === item.id ? t('media.copied') : t('media.copyUrl')}
                  </button>
                  <button
                    onClick={() => remove(item)}
                    className="text-xs px-2 py-1.5 border border-red-200 text-red-600 rounded hover:bg-red-50"
                    aria-label={t('media.deleteLabel', { name: item.originalName })}
                  >
                    ✕
                  </button>
                </div>
